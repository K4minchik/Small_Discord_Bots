...
async def mute(ctx, member: discord.Member, time: int):
    ...
    emb.add_field(name="Длительность (в секундах)", value=f"{time}")
    await ctx.send(embed=emb)
    role = discord.utils.get(member.guild.roles, id=1158012500432924693)
...
async def load(ctx):
    if ctx.message.attachments:
        for attachment in ctx.message.attachments:
            ...
            await ctx.send(detect_bird(f"./images/{file_name}", "./converted_keras/keras_model.h5", "./converted_keras/labels.txt"))
    else:
        await ctx.send("Вы забыли загрузить файл")
...
async def help(ctx):
    ...
    emb.add_field(name="%password (число символов)", value="Генерирует пароль (макс. 2000 символов)", inline=False)
    emb.add_field(name="%coin", value="Жребий", inline=False)
    emb.add_field(name="%joined @member", value="Выводит когда зашёл на сервер указанный пользователь", inline=False)
    ...
    emb.add_field(name="%load", value="Загружает файл и проверяет его на наличие в одном из 4 классов (Синички, Орлы, Голуби, Воробьи)", inline=False)

    emb.add_field(name="  Комманды Админа", value="")
    emb.add_field(name="%mute @member (секунды)", value="Мутит учасника на кол-во секунд", inline=False)
